...
[2024-04-16 17:50:32,567] ERROR in app: Exception on /opl/search-to-view-products [GET]
...
psycopg2.errors.GroupingError: column "product.product_name" must appear in the GROUP BY clause or be used in an aggregate function
LINE 1: ...oduct.product_id AS brand_opl_product_product_id, brand_opl....
                                                             ^
...
Traceback (most recent call last):
  ...
  File "/home/gtrivedi/.local/lib/python3.12/site-packages/flask/app.py", line 852, in dispatch_request
    return self.ensure_sync(self.view_functions[rule.endpoint])(**view_args)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/home/gtrivedi/git/gitlab/opl-ui/routes/view_routes.py", line 41, in view_products
  ...
  File "/home/gtrivedi/.local/lib/python3.12/site-packages/sqlalchemy/engine/default.py", line 922, in do_execute
    cursor.execute(statement, parameters)
sqlalchemy.exc.ProgrammingError: (psycopg2.errors.GroupingError) column "product.product_name" must appear in the GROUP BY clause or be used in an aggregate function
LINE 1: ...oduct.product_id AS brand_opl_product_product_id, brand_opl....
                                                             ^

[SQL: SELECT brand_opl.product.product_id AS brand_opl_product_product_id, brand_opl.product.product_name AS brand_opl_product_product_name, brand_opl.product.product_status AS brand_opl_product_product_status, brand_opl.product.last_updated AS brand_opl_product_last_updated, array_agg(brand_opl.product_portfolios.category_name) AS portfolio_names 
FROM brand_opl.product LEFT OUTER JOIN brand_opl.product_portfolio_map ON brand_opl.product.product_id = brand_opl.product_portfolio_map.product_id LEFT OUTER JOIN brand_opl.product_portfolios ON brand_opl.product_portfolio_map.category_id = brand_opl.product_portfolios.category_id GROUP BY brand_opl.product.product_id ORDER BY brand_opl.product.product_name]
