...
Kind and kubectl along with podman or docker
git
openssl
...
The UI will be available at https://localhost:9443. You can login using the test user.
username: user1
...
Start a new channel in smee, and take a note of the webhook proxy URL.

Create a GitHub app following Pipelines-as-Code documentation.
...
per the instructions on the link, generate and download the private key and create a secret on the cluster providing the location of the private key, the App ID, and the openssl-generated secret created during the process.

To allow Konflux to send PRs to your application repositories, the same secret should be created inside the build-service and the integration-service namespaces. See additional details under Configuring GitHub Application Secrets.

Deploy the smee-client on the cluster:
...
Configuring a push secret that will allow the build pipeline to push images to Quay.io for namespace user-ns2. For that, follow the procedure for configuring the push secret.
Creating an organization and an application in Quay.io that will allow Konflux to create repositories for your applications. To do that, Follow the procedure to configure a Quay.io application and deploy image-controller.
Create Application and Component via the Konflux UI
Follow these steps to onboard your application:

Login to Konflux as user2.
Click Create application
Provide a name to the application and click "Add a component"
Under Git repository url, copy the https link to your fork. This should be something similar to https://github.com/<your-name>/testrepo.git.
Under the Pipeline drop-down list, select docker-build.
Click Create application.
...
Use a text editor to edit your local copy of the example application manifests:
...
Log into the Konflux UI as user2 (password: password). You should be able to see your new Application and Component by clicking "View my applications".
Image Registry
The build pipeline that you're about to run pushes the image it builds to an image registry.
...
Tekton will trigger pipelines present in the .tekton directory. The pipelines already exist on your repository, you just need to copy them to that location:
mkdir -p .tekton
cp pipelines/* .tekton/
...
Your terminal should now display a link for creating a new Pull Request in GitHub. Click the link, make sure the PR is targeted against your fork's main branch and not against the repository from which it was forked (i.e. base repository should reside under your user name).
...
Local Registry
if using a local registry, Port-forward the registry service, so you can reach it from outside of the cluster:
...
You can now list the tags on that test-component repository (assuming you did not change the pipeline's output-image parameter):
...
Pull the image starting with on-pr- (we use podman below, but the commands should be similar on docker):
...
Create a push secret based on your login information and deploy it to namespace user-ns2 on the cluster.

Create a new repository on the registry to which your images will be pushed. For example, in Quay.io, you'd need to click the Create New Repository button and provide it with name and location. Free accounts tend to have limits on private repositories, so for the purpose of this example, you can make your repository public.
...
Push your changes to your testrepo fork, either as a new PR or as a change to your previous PR. Observe the behavior as before, and verify that the build pipeline finishes successfully, and that your public repository contains the images pushed by the pipeline.
Configure Integration Tests
You can add integration tests either via the Konflux UI, or by applying the equivalent Kubernetes resource.

In our case, The resource is defined in test/resources/demo-users/user/ns2/ec-integration-test.yaml. You can directly apply it with the following command:
...
Login as user2 and navigate to your application and component.

Click the Integration tests tab.

Click Actions and select Add Integration test.

Fill-in the details from the YAML.
...
You can now post a /retest comment on your GitHub PR, and once the pull-request pipeline is done, you should see your new integration test being triggered alongside the one you had before.
...
You will now configure the on-push pipeline that will be triggered whenever new commits are created on branch main (e.g. when PRs are merged).

Edit the content of the copy you made earlier to the on-push pipeline at .tekton/testrepo-push.yaml, replacing the value of output-image, so that the repository URL is identical to the one previously set for the pull-request pipeline.
...
The ReleasePlan resource includes a reference to the application that the development team wants to release, along with the namespace where the application is supposed to be released.
...
Edit the release plan and verify that the application field contains the name of your application.
...
Under the components mapping list, set the name field so it matches the name of your component and replace <repository url> with the URL of the repository on the registry to which your released images are to be pushed. This is typically a different repository comparing to the one builds are being pushed during tests.
...
If onboarded not using the UI, you'd need to have the repository created on the registry before releases can be pushed to it. See more details on creating repositories in previous steps.
...
Deploy the managed environment team's namespace, along with the resources mentioned above:

kubectl create -k ./test/resources/demo-users/user/managed-ns2
Create a Registry Secret for the Managed Namespace
In order for the release service to be able to push images to the registry, a secret is needed on the managed namespace (managed-ns2). This is the same secret as was previously created on the development namespace (user-ns2).

To do that, follow the instructions for creating a push secret for the release pipeline for namespace managed-ns2.

Trigger the Release
You can now push the changes (if any) to your PR, merge it once the build-pipeline passes and observe the behavior:

Commit the changes you did on your testrepo branch (i.e. introducing the on-push pipeline, in case you did not onboard via the UI) and push them to GitHub.

Once the build-pipeline and the integration tests finish successfully, merge the PR.

On the Konflux UI, you should now see your on-push pipeline being triggered.

Once it finishes successfully, the integration tests should run once more, and a release should be created under the Releases tab.

Wait for the Release to be complete, and check your registry repository for the released image.
...
Konflux offers a comprehensive solution that fortifies your software supply chain against emerging threats. By offering detailed insights into the composition and construction of your software, Konflux enables proactive identification and mitigation of vulnerabilities.

Before tapping into the vast array of benefits offered by Konflux, the initial step involves its installation. The installation of Konflux is structured around seven key procedures:
