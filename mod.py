...
{% block heading %}
<h1 class="pf-v5-c-title pf-m-4xl">Edit products</h1>
{% endblock %}

{% block content %}
<script>
    function resetForm() {
        window.location.href = "{{ url_for('edit_routes.reset_search_form') }}";
    }
</script>
<div class="container">
    ...
    <fieldset class="product-search-group">
        ...
        <div id="search-form" class="my-4">
            <form method="post" action="{{ url_for('edit_routes.edit_products') }}" class="form">
                {{ form.csrf_token }}
                {{ form.hidden_tag() }}
